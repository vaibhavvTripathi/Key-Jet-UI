...
const TypeContextProvider = ({ children }: { children: React.ReactNode }) => {
	...
	const [DisplayTypedParagraph, setDisplayTypedParagraph] = useState<
		Array<Word>
	>([[]]);
	const [KeyPressSound, setKeyPressSound] = useState<boolean>(true);

	useEffect(() => {
		const storedKeyPressSound = Cookies.get('KeyPressSound');
		if (storedKeyPressSound !== undefined) {
			setKeyPressSound(storedKeyPressSound === 'true');
		}
	}, []);
	...
	const HandleKeyDown = (e: KeyboardEvent) => {
		...
		if (KeyPressSound) {
			const audio = new Audio("/KeyPressSounds/analog.wav");
			audio.play();
		}
		console.log("pressed");
		if (keyInput === " ") {
			...
			const updatedUserTypedParagraph: Array<Word> =
				UserTypedParagraph.map((word, index) => {
	...
	const ToggleKeyPressSound = () => {
		if (KeyPressSound) Cookies.set('KeyPressSound', 'false');
		else Cookies.set('KeyPressSound', 'true');
		setKeyPressSound(!KeyPressSound);
	}
